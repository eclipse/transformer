...
public class RegexpSignatureRuleImpl implements SignatureRule {
	...
	public RegexpSignatureRuleImpl(Logger logger,
		...
		if ((packageRenames == null) || packageRenames.isEmpty()) {
			this.packageRenames = Collections.emptyMap();
		} else {
	...
	public String replacePackage(String initialName) {
		return replacePackage(initialName, DOT_WILDCARD, packageRenames);
	}

	@Override
	public String replaceBinaryPackage(String initialName) {
		return replacePackage(initialName, SLASH_WILDCARD, packageRenames);
	}

	private static final String	DOT_WILDCARD	= ".*";
	private static final String	SLASH_WILDCARD	= "/*";

	private String replacePackage(String text, final String wildcard, final Map<String, String> renames) {
		final String initialText = text;

		// does not do exact conversion only as previously
		// but previous approach was missing some values like Hibernate
		// we don't care about the wildcard because the regular expression handles both
		for (final Map.Entry<String, String> renameEntry : renames.entrySet()) {
			final String initialSubValue = renameEntry.getKey();
			final String finalSubValue = renameEntry.getValue();
			final Pattern pattern = Pattern.compile(initialSubValue);
			final Matcher matcher = pattern.matcher(text);
			if (matcher.find()) { // just for the reference equals bellow instead of the string compare
				text = matcher.replaceAll(finalSubValue);
			}
		}
	...
	public String replacePackages(String text) {
		return replacePackages(text, packageRenames);
	}
	...
	private String replacePackages(String text, Map<String, String> renames) {
		...
		// does not do exact conversion only as previously
		// but previous approach was missing some values like Hibernate and wrongly converting JSR 305 annotations
		for (final Map.Entry<String, String> renameEntry : renames.entrySet()) {
			final String initialSubValue = renameEntry.getKey();
			final String finalSubValue = renameEntry.getValue();
			final Pattern pattern = Pattern.compile(initialSubValue);
	...
	public String replaceText(String inputName, String text) {
		...
		if (substitutions == null) {
			// This is now allowed, because of the new
		...
		for (final Map.Entry<String, String> renameEntry : substitutions.entrySet()) {
			final String initialSubValue = renameEntry.getKey();
			final String finalSubValue = renameEntry.getValue();
			final Pattern pattern = Pattern.compile(initialSubValue);
			final Matcher matcher = pattern.matcher(text);
	...
	private String replaceTextDirect(String initialValue, String inputName, Map<String, String> updates, String updateCase) {
		...
		for (final Map.Entry<String, String> entry : updates.entrySet()) {
			final String initialSubValue = entry.getKey();
			final String finalSubValue = entry.getValue();

			// todo: we should build up a map with <String,Pattern> such as the pattern is compiled once
			final Pattern pattern = Pattern.compile(initialSubValue);
			final Matcher matcher = pattern.matcher(finalValue);
			if (matcher.find()) { // just for the reference equals bellow instead of the string compare
				finalValue = matcher.replaceAll(finalSubValue);
